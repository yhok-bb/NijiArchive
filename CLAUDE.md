...
- **将来の課題は `design/backlog.md` に記録する**。開発中に出た検討事項・改善点はissueにせず、まずバックログへ。Step着手時に見返して拾うものだけissue化する
- **機能ごとにPRを作る**。機能実装はブランチを切ってPR経由でマージし、mainに直接コミットしない（ドキュメント・設定の微修正は例外可）。PRは `Closes #<issue番号>` でissueに紐づけ、CIが緑になってからマージする
- **issueの回しかた（学習ループ）**：issueは学習ウェイトをラベルで持つ。`deep`＝学習の中心領域。着手時に本文の「関所（問い）」へissueコメントで回答し、レビューを受けてから実装する。PRレビューも`deep`を重点的に行う。`chore`＝定型作業。委任・省力化を許容する
- **OSS上流へのフィードバック**：gemの挙動が不審なときは、まず自分のバグを疑いつつ、該当gemのissueトラッカーも検索する。再現できたら最小再現スクリプトを作成し、不具合やドキュメントの不備が確認できた場合は上流に報告する（対象領域と貢献の段階はPart 4参照）
